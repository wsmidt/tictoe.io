...
@Singleton
class TicToeController @Inject() (@Named("game-manager") gameManager: ActorRef)(implicit system: ActorSystem, materializer: Materializer) extends Controller with LazyLogging {
  ...
  def connect = WebSocket.accept[InMessage, OutMessage] { request =>
    ActorFlow.actorRef(out => ConnectionActor.props(out, gameManager))
  }
}
